...
public interface ManagerMapper {
    ...
    @Select("select * from sys.user limit #{pageNumber},#{pageSize}")//分页显示数据
    ...
    @Select("select count(*) from sys.user where ${optionSelect} = #{optionValue}")
    Integer selectNum(@Param("optionSelect") String optionSelect, @Param("optionValue") String optionValue);


    @Insert("insert into sys.user(username,sex, password, nickname, email,create_time,update_time)values (#{username},#{sex},#{password}, #{nickname}, #{email},now(), now())")
    ...
    @Insert("insert into sys.user(username,sex, password, nickname, email)values (#{username},#{sex},#{password}, #{nickname}, #{email})")
