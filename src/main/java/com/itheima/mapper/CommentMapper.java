...
public interface CommentMapper {
    @Select("select *from vue.comment where commentable_id=#{id} and commentable_type='feed'")
    List<Comment> getFeedComments(Integer id);

    @Select("select * from vue.comment where target_id = #{id} and commentable_type = 'feed'")
    List<Comment> getFeedCommentByTargetId(Integer id);


    @Select("select *from vue.comment where commentable_id=#{id} and commentable_type='new'")
